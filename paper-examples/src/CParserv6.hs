...
parseBlock = Block
	     <$>
	     (Char.char '{' *> Prim.many (parseCExpr <* Token.semi clexer) <* Char.char '}' )

parseReturn :: Parser Expr
parseReturn = Return
	      <$>
	      (Token.reserved clexer "return" *> Combinator.optionMaybe (parseCExpr) <* Token.semi clexer)
...
parseFunc = Function
	    ...
	    <*>
	    -- need to consider when you only do declaration
	    (Prim.many parseBlock)
...
parseFile :: IO String
parseFile = do
    result <- parseSrc "./src/csources/toy6.c"
